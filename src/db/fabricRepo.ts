...
export async function getAdminIdempotency(key: string, method: string, path: string) {
  const rows = await query<{ request_hash: string; status_code: number; response_json: unknown }>(
    `select request_hash,status_code,response_json
     from admin_idempotency_keys where key=$1 and method=$2 and path=$3 and expires_at > now() limit 1`,
    [key, method, path],
  );
  return rows[0] ?? null;
}

export async function saveAdminIdempotency(key: string, method: string, path: string, hash: string, statusCode: number, responseJson: unknown) {
  await query(
    `insert into admin_idempotency_keys(key,method,path,request_hash,status_code,response_json,expires_at)
     values($1,$2,$3,$4,$5,$6,now()+interval '24 hours') on conflict do nothing`,
    [key, method, path, hash, statusCode, responseJson as object],
  );
}
