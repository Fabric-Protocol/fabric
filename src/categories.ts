...
type LocalizedAppLocale = Exclude<AppLocale, 'en'>;
...
const LOCALIZED_CATEGORY_FIELDS: Record<LocalizedAppLocale, Record<string, LocalizedCategoryFields>> = {
  'zh-Hans': {
    goods: {
      name: '实物商品',
      description: '实体物品',
  ...
  fa: {
    goods: {
      name: 'کالا',
      description: 'اقلام فیزیکی',
      examples: [
        'لباس یا ست مشخص با برند، مدل، سایز و رنگ برای تحویل و رساندن در همان روز',
        'قطعه جایگزین کمیاب برای لوازم خانگی یا خودرو همراه با عکس برچسب یا شماره سریال',
        'خرید محلیِ آیتم محدود و ارسال آن همراه با عکس رسید',
        'آماده‌سازی و پلمب بسته رسانه فیزیکی مانند USB یا درایو همراه با عکس زنجیره تحویل',
        'تحویل بسته راستی‌آزمایی اصالت دستگاه یا قطعه با عکس‌های استاندارد و فهرست بسته‌بندی',
      ],
    },
    services: {
      name: 'خدمات',
      ...
      examples: [
        'مراجعه دو ساعته تعمیرکار برای مونتاژ، نصب یا ترمیم همراه با عکس قبل و بعد',
        'نظافت عمیق اتاق‌های مشخص همراه با مدرک تکمیل کار',
        'حضور در محل برای کارهای ساده روی دستگاه مانند ریبوت، تعویض کابل، خواندن چراغ‌ها یا جابه‌جایی',
        'مراقبت از کار طولانی مثل پرینت سه‌بعدی، CNC یا بکاپ همراه با قوانین مداخله و گزارش‌ها',
        'ایستادن در صف و انجام تحویل حضوری همراه با مدرک زمان حضور در صف',
      ],
    },
    space_asset_time: {
      name: 'زمان فضا و دارایی',
      description: 'اجاره، امانت یا استفاده',
      examples: [
        'استفاده از جای پارک یا ورودی خودرو در یک بازه زمانی مشخص',
        'زمان استفاده از بخش کارگاه با حضور فروشنده و تحویل خروجی',
        'بازه استفاده از محل امن برای نگهداری و تحویل گرفتن یا تحویل دادن همراه با قوانین و زمان‌ها',
        'گوشه انبار کوتاه‌مدت با برنامه دسترسی مشخص و فهرست تصویری هنگام ورود و خروج',
        'یک ساعت اتاق یا استودیو آرام با نیازهای برق و وای‌فای و روش دسترسی مشخص',
      ],
    },
    access_reservations: {
      name: 'دسترسی و رزروها',
      description: 'رویدادها، صف‌ها و عضویت‌ها',
      examples: [
        'انتقال رزرو رستوران یا تغییر نام مهمان در جایی که مجاز است',
        'دعوت‌نامه مهمان یا سهمیه همراه برای رویداد',
        'بازه نوبت قابل انتقال در جایی که سیاست اجازه می‌دهد',
        'ورود اولویت‌دار یا هماهنگی میزبانی همراه با محدودیت‌های روشن و مدرک تایید',
        'ارسال از مسیر عضویت فروشنده در جایی که قانونی است همراه با مدرک ارسال و تایید',
      ],
    },
    logistics_transportation: {
      name: 'لجستیک و حمل‌ونقل',
      description: 'افراد، کالاها و کارهای بیرونی',
      examples: [
        'تحویل گرفتن و رساندن همراه با رسیدها و رهگیری',
        'بسته‌بندی و ارسال با مواد لازم و مدرک تصویری',
        'رله پیک پلمب‌شده با نوار امنیتی، زمان‌ها و عکس‌های تحویل',
        'ارسال زنجیره سرد همراه با ثبت دما',
        'رله چندمرحله‌ای بین شهرها همراه با چک‌لیست استاندارد تحویل و بسته مدرک',
      ],
    },
    proof_verification: {
      name: 'اثبات و راستی‌آزمایی',
      description: 'بازرسی، گواهی و زنجیره نگه‌داری',
      examples: [
        'بازرسی وضعیت با عکس‌ها، اندازه‌ها و رسیدها',
        'راستی‌آزمایی وضعیت واقعی آپارتمان از نظر صدا، بو یا پارکینگ در ساعات شلوغ همراه با مدرک زمان‌دار',
        'اثبات حضور در بازه زمانی مشخص با رمز یا حرکت توافق‌شده و مدرک زمان‌دار',
        'غربال اولیه اصالت از طریق شماره سریال و نشانه‌های بسته‌بندی همراه با چک‌لیست عکس استاندارد',
        'بسته مدرک زنجیره نگه‌داری از تحویل پلمب‌شده تا ثبت تحویل‌ها و اثبات رساندن',
      ],
    },
    account_actions_delegated_access: {
      name: 'اقدام‌های حساب و دسترسی واگذارشده',
      description: 'اقدام‌های حساب و دسترسی واگذارشده',
      examples: [
        'ثبت، مطالبه یا بازخرید با عضویت یا حساب فروشنده در محدوده مشخص و با رضایت',
        'ارسال، فهرست‌کردن یا اجرای یک مورد در کانال یا فضای کاری متعلق به فروشنده',
        'دسترسی موقت ادمین یا افزودن به فضای کاری که قابل لغو و زمان‌دار باشد',
        'صدور کلید دسترسی یا توکن مجوز زمان‌دار و لغو آن طبق برنامه همراه با مدرک',
        'ارسال اولویت‌دار در پرتال محدود قابل دسترس برای فروشنده همراه با آثار تایید',
      ],
    },
    digital_resources: {
      name: 'منابع دیجیتال',
      description: 'محاسبات، ذخیره‌سازی و زیرساخت',
      examples: [
        'ساعت GPU با مدل دقیق و هش کانتینر قابل بازتولید',
        'سهمیه ذخیره‌سازی یا پهنای باند برای مدت ثابت',
        'گیرنده webhook میزبانی‌شده با تلاش مجدد و لاگ‌ها به‌صورت زمان‌دار',
        'نقطه پایانی callback اختصاصی و خروجی لاگ حسابرسی برای جریان‌های کاری عامل‌ها',
        'رانر اجرایی مخصوص منطقه برای محدودیت‌های تاخیر یا مقررات همراه با لاگ اجرا و هش‌ها',
      ],
    },
    rights_ip: {
      name: 'حقوق و مالکیت فکری',
      description: 'مجوزها، اجازه‌ها و حقوق دیجیتال کمیاب',
      examples: [
        'دسترسی زمان‌دار به یک مجموعه‌داده منحصربه‌فرد با شرایط مجوز روشن',
        'مجوز استفاده از یک عکس یا دارایی در دامنه محدود',
        'آزادسازی یک‌باره کلید رمزگشایی در زمان برنامه‌ریزی‌شده با کنترل حقوق',
        'انتقال یا واگذاری مجوز در جایی که قانونی است همراه با بسته مستندات زنجیره مالکیت',
        'اقلام مجازی یا کلکسیون‌های دیجیتال از جمله NFT در جایی که شرایط سرویس انتقال را اجازه می‌دهد',
      ],
    },
    social_capital_communities: {
      name: 'سرمایه اجتماعی و اجتماع‌ها',
      description: 'معرفی‌ها، اعتبار و جامعه‌ها',
      examples: [
        'معرفی گرم مشروط بر اساس معیارها و تحت کنترل فروشنده',
        'تایید یا معرفی با دامنه صریح',
        'دعوت به اجتماع خصوصی همراه با پیام پشتیبان',
        'جایگاه توزیع: فروشنده درخواست یا آیتم شما را با نام خود در یک گروه مورد اعتماد منتشر می‌کند',
        'طرح یک پرسش یا درخواست در حلقه متخصصانی که فروشنده کنترل می‌کند همراه با خلاصه پاسخ‌ها و مدرک حضور',
      ],
    },
  },
};
...
export function getCategoriesResponse(locale: AppLocale = 'en'): CategoriesResponse {
  ...
  const localizedFields = LOCALIZED_CATEGORY_FIELDS[locale];
  if (!localizedFields) return CATEGORIES_RESPONSE;
  return {
    categories_version: CATEGORIES_VERSION,
    categories: CATEGORIES.map((category) => {
      const localized = localizedFields[category.slug];
