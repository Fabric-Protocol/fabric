...
test('GET /v1/meta returns required legal version and legal URLs', async () => {
  ...
  assert.ok(toc.invariants.includes('webhook_hmac_sha256_signing_optional'));
  assert.ok(toc.invariants.includes('webhook_retry_exponential_backoff_bounded_window'));
  assert.ok(toc.invariants.includes('webhook_no_challenge_response_handshake'));
...
test('GET /openapi.json returns valid OpenAPI JSON', async () => {
  ...
  const bootstrapPost = body.paths?.['/v1/bootstrap']?.post ?? {};
  const bootstrapParams = bootstrapPost.parameters ?? [];
  assert.equal(bootstrapParams.some((p) => p && p.$ref === '#/components/parameters/IdempotencyKeyHeader'), true);
  assert.equal(bootstrapPost.requestBody?.content?.['application/json']?.schema?.$ref, '#/components/schemas/BootstrapRequest');
  assert.equal(bootstrapPost.responses?.['200']?.content?.['application/json']?.schema?.$ref, '#/components/schemas/BootstrapResponse');
  const recoveryStartPost = body.paths?.['/v1/recovery/start']?.post ?? {};
  const recoveryStartParams = recoveryStartPost.parameters ?? [];
  assert.equal(recoveryStartParams.some((p) => p && p.$ref === '#/components/parameters/IdempotencyKeyHeader'), true);
  assert.equal(recoveryStartPost.requestBody?.content?.['application/json']?.schema?.$ref, '#/components/schemas/RecoveryStartRequest');
  assert.equal(recoveryStartPost.responses?.['200']?.content?.['application/json']?.schema?.$ref, '#/components/schemas/RecoveryStartResponse');
  const recoveryCompletePost = body.paths?.['/v1/recovery/complete']?.post ?? {};
  const recoveryCompleteParams = recoveryCompletePost.parameters ?? [];
  assert.equal(recoveryCompleteParams.some((p) => p && p.$ref === '#/components/parameters/IdempotencyKeyHeader'), true);
  assert.equal(recoveryCompletePost.requestBody?.content?.['application/json']?.schema?.$ref, '#/components/schemas/RecoveryCompleteRequest');
  assert.equal(recoveryCompletePost.responses?.['200']?.content?.['application/json']?.schema?.$ref, '#/components/schemas/RecoveryCompleteResponse');
  const mePatchOp = body.paths?.['/v1/me']?.patch ?? {};
  const callbackExpr = '{$request.body#/event_webhook_url}';
  const webhookCallbackPost = mePatchOp.callbacks?.eventWebhookDelivery?.[callbackExpr]?.post ?? {};
  assert.equal(webhookCallbackPost.requestBody?.content?.['application/json']?.schema?.$ref, '#/components/schemas/EventWebhookDelivery');
  const webhookCallbackParams = webhookCallbackPost.parameters ?? [];
  assert.equal(webhookCallbackParams.some((p) => p && p.$ref === '#/components/parameters/EventWebhookTimestampHeader'), true);
  assert.equal(webhookCallbackParams.some((p) => p && p.$ref === '#/components/parameters/EventWebhookSignatureHeader'), true);
  const webhookPayloadSchema = body.components?.schemas?.EventWebhookDelivery?.properties?.payload ?? {};
  assert.equal(webhookPayloadSchema.additionalProperties, false);
  const recoveryStartRequestSchema = body.components?.schemas?.RecoveryStartRequest ?? {};
  assert.equal(recoveryStartRequestSchema.properties?.method?.enum?.includes('pubkey'), true);
  assert.equal(recoveryStartRequestSchema.properties?.method?.enum?.includes('email'), true);
  const bootstrapRequestSchema = body.components?.schemas?.BootstrapRequest ?? {};
...
test('GET /docs/agents returns quickstart HTML content', async () => {
  ...
  assert.match(res.body, /Required for notifications/);
  assert.match(res.body, /PATCH[\s\S]*\/v1\/me/);
