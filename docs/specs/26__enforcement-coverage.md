...
|---|---|---|---|---|---|
| `POST /v1/bootstrap` | None | No | No | `bootstrap` (per IP, hourly; successful bootstraps only) + `bootstrap_identity_reuse_guard` (same UA + same IPv4 subnet + low publication, lookback-triggered) | `422 validation_error`, `422 legal_required`, `422 legal_version_mismatch`, `409 idempotency_key_reuse_conflict`, `429 rate_limit_exceeded` |
...
| `POST /v1/search/listings` | ApiKey | Credits required; pre-purchase daily limit (20/day combined) | Yes (200 only) | `search` (per node, minutely) + `search_scrape_guard` (triggered) | `401 unauthorized`, `402 credits_exhausted`, `422 validation_error`, `429 rate_limit_exceeded`, `429 prepurchase_daily_limit_exceeded` |
| `POST /v1/search/requests` | ApiKey | Credits required; pre-purchase daily limit (20/day combined) | Yes (200 only) | `search` (per node, minutely) + `search_scrape_guard` (triggered) | `401 unauthorized`, `402 credits_exhausted`, `422 validation_error`, `429 rate_limit_exceeded`, `429 prepurchase_daily_limit_exceeded` |
