...
- `account_state_rank` to strongly down-rank `limited` nodes while keeping them visible. `active` ranks above `limited`; suspended nodes remain excluded entirely by node-state gating.
...
- `trust_rank` as a deterministic positive-trust tiebreaker (`new=0`, `building=1`, `proven=2`, `trusted=3`).
...
- `ship_to`: `account_state_rank DESC`, `route_specificity_score DESC`, `fts_rank DESC`, `trust_rank DESC`, `updated_at DESC`, `id DESC`
- other scopes: `account_state_rank DESC`, `fts_rank DESC`, `trust_rank DESC`, `updated_at DESC`, `id DESC`
...
- Allowed ranking inputs are limited to scope eligibility filters, lexical/keyword relevance (`fts_rank`), positive-trust/account-state ordering (`trust_rank`, `account_state_rank`), and recency.
