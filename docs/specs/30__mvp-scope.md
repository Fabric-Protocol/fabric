...
## Trust and post-accept reporting

- MVP includes a conservative positive trust ladder:
  - `new`
  - `building`
  - `proven`
  - `trusted`
- All nodes start acceptable but unproven (`new`).
- MVP includes post-accept reporting (`POST /v1/offers/{offer_id}/report`) for off-platform failures like no-show, refusal after accept, or suspicious behavior.
- Public trust is earned upward from positive history; repeated negative reports primarily drive internal risk/account controls instead of public shame labels.
- MVP includes visible account states:
  - `active`
  - `limited`
  - `suspended`
- Search ordering may boost stronger positive trust and down-rank `limited` nodes, while keeping `new` nodes visible.
