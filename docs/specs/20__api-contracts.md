...
```json
{
  ...
  "email": "string|null (optional; omitted => null)",
  "referral_code": "string|null (optional; omitted => null)",
...
`setup_incomplete` is optional. When present, it contains follow-up actions that are strongly recommended before real usage, especially persisting the bootstrap credentials, configuring `recovery_public_key`, and configuring `event_webhook_url`.

`node.recovery_public_key_configured=false` means the node is not recoverable if the API key is lost. Clients SHOULD treat that as a blocking onboarding task and set `recovery_public_key` immediately via bootstrap or `PATCH /v1/me`.
...

{
  "node": {
    ...
    "event_webhook_url": "https://example.com/webhook|null",
    "status": "ACTIVE|SUSPENDED",
    "account_state": "active|limited|suspended",
    "account_state_reasons": ["string"],
    "trust": {
      "trust_tier": "new|building|proven|trusted",
      "mutual_accepts_total": 0,
      "unique_counterparties_total": 0,
      "response_rate_30d": 0,
      "response_rate_90d": 0,
      "response_rate_band": "unknown|low|fair|good|excellent",
      "response_speed_band": "unknown|fast|steady|slow",
      "report_incidence_band": "none|low|elevated",
      "reasons": ["string"],
...
`setup_incomplete` is optional. When present, it contains follow-up actions that are strongly recommended before real usage (for example configuring `recovery_public_key`, `event_webhook_url`, or an email for post-accept contact).
...
Not required on the `GET` variants. The `POST` variants below require it.
...
Create a Unit. Public-by-default: if `publish_status` is omitted, behavior is equivalent to `publish_status="published"`. Publish-ready payloads auto-publish immediately; payloads that are not publish-ready are saved as drafts because they cannot be public yet. Send `publish_status="draft"` only when you intentionally want a private draft.
...
`type` is a free-form classifier string (for example `service`, `goods`, `compute`, `dataset_access`). It is not an enum, but it MUST be non-null before publication.
...
Create a Request. Public-by-default: if `publish_status` is omitted, behavior is equivalent to `publish_status="published"`. Publish-ready payloads auto-publish immediately; payloads that are not publish-ready are saved as drafts because they cannot be public yet. Send `publish_status="draft"` only when you intentionally want a private draft.
...
`type` remains a free-form classifier string here as well; it is not an enum, but it MUST be non-null before publication.
...
`ttl_minutes` may be provided on patch. Same bounds [60, 525600] apply; server recomputes and returns `expires_at`.
...
These are metered reads (credit spend) for authenticated ACTIVE, not-suspended nodes.
...
POST /v1/public/nodes/{node_id}/listings/categories/{category_id}

Same response and error semantics as the GET drilldown, but accepts the pagination and budget payload in the body:
{
  "limit": 20,
  "cursor": "string|null",
  "budget": {
...
POST /v1/public/nodes/{node_id}/requests/categories/{category_id}

Same semantics and shape as the listings POST drilldown.
...
12a) Post-accept reporting (pattern-based trust input)
...
Not required on the `GET` variants. The optional `POST` drilldown variants below use normal non-GET idempotency semantics.
...
Record a structured post-accept report about failed follow-through or suspicious behavior. This is signal collection for trust/risk patterns, not arbitration.

Request
{
  "reason": "no_show|unresponsive|refused_after_accept|fraud_suspected|other",
  "note": "string|null",
...
- Only parties to the offer may report.
- Offer must already be `mutually_accepted`.
- At least one contact reveal must already exist for the offer.
- One report per side per offer.
- Reporting window closes `REPORT_WINDOW_DAYS` after the first contact reveal for the offer (or mutual acceptance if reveal timestamps are unavailable).
- Reports feed public trust conservatively and internal account/risk controls more strongly.
...
409 invalid_state_transition (`details.reason` in `offer_not_reportable|contact_reveal_required_for_report|report_window_expired|offer_report_already_exists`)

422 validation_error

12b) Node lifecycle events (webhook + polling fallback)
...
Poll node lifecycle events for the authenticated node using cursor pagination.
...
Response 200
{
  "events": [
    {
      "id": "uuid",
      "type": "offer_created|offer_countered|offer_accepted|offer_rejected|offer_cancelled|offer_contact_revealed|subscription_changed|credits_topup_completed|credits_topup_failed",
      "offer_id": "uuid",
      "actor_node_id": "uuid",
...
14c) Billing auto-topup setup session
...
Create a Stripe Checkout Session in `setup` mode to save a card for future off-session Credit Pack purchases.
...
- `node_id` must match the authenticated Node.
- Checkout is card-only for this flow.
- If the Node already has a mapped Stripe customer, the setup session reuses it; otherwise Stripe creates one.
- Creates Stripe Checkout Session in `setup` mode and sets:
  - `metadata.node_id`
  - `metadata.purpose="auto_topup_setup"`
  - `setup_intent_data.metadata.node_id`
  - `setup_intent_data.metadata.purpose="auto_topup_setup"`
- On `checkout.session.completed` for this setup flow, the server stores the resulting Stripe customer id and saved card payment method summary for future off-session Credit Pack purchases.
...
14d) Billing auto-topup configuration
...
Inspect or update the low-balance auto-topup rule for the authenticated Node.

GET Response 200
{
  "enabled": false,
  "threshold_credits": "int|null",
  "pack_code": "credits_500|credits_1500|credits_4500|null",
  "monthly_spend_cap_cents": "int|null",
  "month_to_date_spend_cents": 0,
  "has_saved_payment_method": true,
  "payment_method": { "type": "card", "brand": "visa", "last4": "4242" } | null,
  "last_charge_attempt_at": "timestamp|null",
  "last_charge_status": "none|processing|succeeded|failed|requires_payment_method|requires_action",
  "last_payment_intent_id": "pi_...|null",
...
POST Rules / side effects

- `threshold_credits` must be an integer `>= 0`.
- `threshold_credits` must be strictly less than the selected pack's credit amount, so one successful auto-topup exits the low-balance condition.
- `monthly_spend_cap_cents`, when present, must be an integer `>= 0`.
- `monthly_spend_cap_cents`, when present while `enabled=true`, must be at least the selected pack price in cents.
- `monthly_spend_cap_cents` applies to successful auto-topup spend in the current UTC calendar month.
- Omitting `monthly_spend_cap_cents` preserves the current cap; sending `null` clears it.
- Enabling auto-topup requires a saved Stripe card payment method and mapped Stripe customer.
- Auto-topup attempts use the existing Credit Pack catalog and `topup_purchase` ledger semantics; no new SKU class is introduced.
- Triggering rule: after a successful credit debit, if balance is `<= threshold_credits`, Fabric may attempt one off-session Stripe PaymentIntent for the configured pack.
- Safety rails in this first slice:
  - existing `CREDIT_PACK_MAX_GRANTS_PER_DAY` still applies before attempting an auto-topup charge
  - server-enforced cooldown between auto-topup attempts to avoid repeated failed charges
  - while a prior auto-topup PaymentIntent is still `processing`, Fabric does not start another attempt for that rule until Stripe reconciliation finishes
  - delayed Stripe reconciliation is handled through PaymentIntent webhooks so `processing` attempts can later settle to success or failure
- Successful auto-topup grants emit `credits_topup_completed` node events.
- Failed or blocked auto-topup attempts emit `credits_topup_failed` node events.

POST Response 200

Same shape as `GET /v1/billing/auto-topup`.

Errors

409 invalid_state_transition (`reason=auto_topup_payment_method_required|auto_topup_customer_required`)

422 validation_error
  - `reason=unsupported_pack_code`
  - `reason=auto_topup_threshold_invalid`
  - `reason=auto_topup_threshold_must_be_below_pack_credits`
  - `reason=auto_topup_monthly_spend_cap_invalid`
  - `reason=auto_topup_monthly_spend_cap_too_low`

DELETE Rules / side effects

- Disables auto-topup for the authenticated Node.
- Clears Fabric-stored saved-card metadata (`stripe_payment_method_id`, brand/type/last4, and recent attempt status).
- Does **not** detach the payment method from Stripe. This path is Fabric-local only for MVP safety.
...
payment_intent.processing|succeeded|payment_failed|requires_action
...
- `checkout.session.completed` in `mode=setup` stores the Stripe customer id and saved card payment method summary for future auto-topup use.
- Auto-topup PaymentIntent events reconcile delayed off-session outcomes:
  - `payment_intent.processing` keeps the attempt in `processing`
  - `payment_intent.succeeded` grants credits idempotently and emits `credits_topup_completed`
  - `payment_intent.payment_failed` / `payment_intent.requires_action` record failure state and emit `credits_topup_failed`
...
GET /v1/admin/diagnostics/stripe
Auth
...
Return the current Stripe configuration and price-map readiness snapshot used by billing flows.

Response 200
{
  "stripe_configured": true,
  "missing": ["STRIPE_SECRET_KEY"],
  "price_id_counts_by_plan": {
    "basic": 1,
    "pro": 1,
    "business": 1
  },
  ...
  "active_base_url": "https://api.fabric.example"
}
...
POST /v1/admin/nodes/{node_id}/api-keys
Auth
...
Mint a replacement or post-verification API key for a specific node.
...
POST /internal/admin/health-pulse
Auth
...
Not required (scheduler-friendly internal endpoint)
...
Return a short-window operational pulse for Stripe processing, crypto payment status, and webhook retry backlog. May also send ops alerts when degraded.
...
POST /internal/admin/daily-digest
Auth
...
Generate the daily operational digest, log it, post to Slack, and optionally email it.

Response 200
{
  "ok": true,
  "email_sent": false,
  "email_provider": "stub|smtp|sendgrid|resend|none",
  ...
  "metrics": {
    "generated_at": "iso",
    "window_hours": 24,
    "abuse": { "suspended_nodes": 0, "active_takedowns": 0, "recovery_attempts_exceeded": 0 },
    "stripe_credits_health": { "stripe_events_received": 0, "stripe_processing_errors": 0, "credit_grants": 0, "credit_debits": 0, "credit_net": 0 },
    "liquidity": { "public_listings": 0, "public_requests": 0, "offers_created": 0, "offers_mutually_accepted": 0 },
    "reliability": { "searches": 0, "active_nodes": 0, "active_api_keys": 0 },
    "webhook_health": { "stripe_events_received": 0, "stripe_processing_errors": 0, "offer_webhook_deliveries": 0, "offer_webhook_failures": 0 }
  }
}

POST /internal/admin/sweep
Auth
...
Expire stale offers and requests, release associated holds, and refresh trust as needed.
...
Delete search-log rows past the retention delete cutoff and report the cutoffs used.
...
POST /v1/admin/nodes/{node_id}/account-state
Auth
...
Set a node's visible account state for operational enforcement.
